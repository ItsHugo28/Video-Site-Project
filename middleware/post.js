...
module.exports = {
    ...
    getPostsForUserBy: async function (req, res, next) {
        ...
        try {
            let [rows, _] = await db.execute(
                `SELECT p.id, p.title, p.description, p.createdAt, p.thumbnail
        FROM posts p
        JOIN users u ON p.fk_userid = u.id
        WHERE u.id = ?`, [id]);
            res.locals.userPost = rows;
            next();
        } catch (error) {
            next(error);
        }
    ...
    getPostById: async function (req, res, next) {
      ...
      try{
          ...
          const post = rows[0];
          if(!post){
              req.flash("error", 'Not a valid post');
    ...
    getRecentPosts: async function (req,res,next){
        try{
            const query = 'SELECT * FROM csc317db.posts ORDER by createdAt DESC LIMIT 8'
            const [results] = await db.query(query);
            res.locals.recentPost = results;
            next();
        }catch (error){
            next(error);
        }
